...
| **Resultado da votação** | Apuração dos votos de uma sessão já fechada: total SIM, total NÃO e status (APROVADA, REJEITADA, EMPATE). |
...
| R13 | O status do resultado é definido pela maioria: SIM > NÃO (APROVADA), SIM < NÃO (REJEITADA), empate (EMPATE). |
